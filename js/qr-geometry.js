 ...
 *
 * ── 位置づけ ────────────────────────────────────────────────────
 *   本ファイルは §2（物理レイアウト）を担う最下層。§3 以降のモジュール
 *   （qr-align.js / qr-version.js / qr-core.js …）がこの箱定数とセル寸法を
 *   土台にする。指示書 §0〜§11 は現在すべて実装済み（README「実装状況」参照）。
 *   ※ かつてここには「§5 以降は保留」と書かれていたが、当時の作業スコープを
 *     示す記述であり、現在の実装状況とは無関係のため更新した。
 *
 * ── 標準ティアと拡張ティア ──────────────────────────────────────
 *   MIN_CELL_MM(0.7mm)     … 標準ティア（ver1〜14・300dpi）の硬い下限。
 *   MIN_CELL_EXT_MM(0.5mm) … 拡張ティア（ver15〜20）の硬い下限。こちらは
 *     600dpi 印刷/スキャンを前提にすることで、1 セルあたりの画素数を
 *     300dpi/0.7mm（8.27px）より **多く** 確保する（下記の議論を参照）。
 *   requiredDpi()          … その cols×rows が要求する印刷/スキャン解像度。
 *     UI（creator.html の警告・描画倍率）はこの値を使うこと。
 *     「ver15 以上は 600dpi」と決め打ちしてはいけない（ver15 は 300 を返す）。
 *
 * 詳細な仕様は docs/FORMAT.md §1、設計理由は docs/ARCHITECTURE.md を参照。
 * ================================================================== */
